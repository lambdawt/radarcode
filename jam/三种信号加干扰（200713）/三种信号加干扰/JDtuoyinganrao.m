function [s_ft,echo3]=JDtuoyinganrao(R,frame,tf,Aj,tm,tau,f0,N,f_doppler1,f_doppler,vf,lamta,ts,c,fs,temp1)
CPI=0;
P=0;
if temp1 == 3%联合
    for i=0:frame-1
       ...
       % echo3(i+1,:)=Aj*rectpuls(tm-delt1-P-tau/2,tau).*exp(j*2*pi*(((f0-B1/2)+k*(tm-delt1-P)/2).*(tm-delt1-P)));
        echo3(i+1,:)=Aj*rectpuls(tm-2*(R)/c-tau/2-P,tau).*exp(1j*2*pi*(((f0)).*(tm-2*R/c-P)));
    end
    for i=1:frame
