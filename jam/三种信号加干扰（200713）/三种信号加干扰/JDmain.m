%简单脉冲主函数
function []=JDmain(fz,B,Gt,Gr,F,Pt,L,Pfa,R,v,rcsk,sigma0,Te,tau,tr,f0,f1,fs,frame,num_jilei,num_tongdao,num_cankao,num_baohu) 
% clear all;
% close all;
% clc;
global c;
...
figure,plot((0:fs/length(abs(fft(abs(fftshift(echo(1,:)))))):fs-fs/length(abs(fft(abs(fftshift(echo(1,:))))))),abs((fft(echo(1,:))))),xlabel('频率f(单位：Hz)'), ylabel('y(单位：伏/Hz)'),title('简单脉冲回波信号的频谱');
%生成噪声
...
figure,mesh(1:fr/num_tongdao:fr,0:ts*c/2:(length(abs(s_mtd(:,1)))*ts-ts)*c/2,abs(s_mtd)),xlabel('多普勒频率，单位：Hz'),ylabel('距离，单位：米'),zlabel('y(单位：伏)'),title('JD MTD结果');
