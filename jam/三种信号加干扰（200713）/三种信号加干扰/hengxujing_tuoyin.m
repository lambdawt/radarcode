function []=hengxujing_tuoyin(M1,Pfa,s_mtd,num_cankao,num_tongdao,num_baohu,ts,c,tau,D,s_mtd1)
N=M1;
Kc=sqrt(-2*log(Pfa));
...
for i=1:num_tongdao
    ...
    subplot(4,num_tongdao/4,v),plot(0:ts*(tau/D/ts)*c/2:(length(abs(cfar_k_result))*ts*(tau/D/ts)-ts)*c/2,abs(cfar_k_result)),xlabel('距离，单位：米'),ylabel('y(单位：伏)');axis([0 inf 0 cfarmax]); 
...
figure,
cfarmax=max(max(abs(s_mtd1)));
for i=1:num_tongdao
    v=i;
    s_mtd_1=abs(s_mtd1(:,i));
    s_pc1=s_mtd_1.';
    %     N=4096;
    cfar_k_result(1,1)=mean(s_pc1(1,2:num_cankao+1));
    if s_pc1(1,1)>=sqrt(2/pi)*mean(s_pc1(1,2:num_cankao+1))*Kc
