%主函数
function []=LFMmain(fz,B,B1,Gt,Gr,F,Pt,L,Te,tau,tr,f0,f1,fs,frame,num_jilei,num_tongdao,num_cankao,num_baohu,Pfa,R,v,rcsk,sigma0) 
% clear all;
%  close all;
% clc;
global c;
...
figure,plot((0:fs/length(abs(fft(abs(fftshift(y))))):fs-fs/length(abs(fft(abs(fftshift(y)))))),abs(fft(y))),xlabel('频率f(单位：Hz)'), ylabel('y(单位：伏/Hz)'),title('LFM发射信号的频谱');
%生成理想脉冲压缩系数
[M,match_filter_fft]=maiyaxishu(f0,fs,y/sqrt(Pt),tr,ts,N);

%生成回波信号
[s_echo_2,echo]=LFMhuiboxinhao(R,c,A,N,frame,fs,f_doppler,tm,f0,B1,tau,k); 
figure,plot(0:ts:(N-1)*ts,real(s_echo_2(1:N))),xlabel('t(单位：s)'), ylabel('y(单位：伏)'),title('LFM回波信号');
figure,plot((0:fs/length(abs(fft(abs(fftshift(echo(1,:)))))):fs-fs/length(abs(fft(abs(fftshift(echo(1,:))))))),abs((fft(echo(1,:))))),xlabel('频率f(单位：Hz)'), ylabel('y(单位：伏/Hz)'),title('LFM回波信号的频谱');
%生成噪声
[s_noise]=zaosheng(frame,N,An,B,fs);
%目标回波信号、假目标信号、噪声叠加在一起送入接收机
s_echo_1=s_echo_2+s_noise;
% figure,plot(0:ts:(N-1)*ts,real(s_echo_1(1:N))),xlabel('t(单位：s)'), ylabel('y(单位：伏)'),title('LFM回波信号');
%高放
...
% figure,plot(0:ts:(N-1)*ts,real(s_echo_mf(1:N))),xlabel('t(单位：s)'), ylabel('y(单位：伏)'),title('LFM回波信号');
%脉冲压缩及降采样s_echo_1
...
% figure,mesh(1:fr/num_tongdao:fr,0:ts*(tau/D/ts)*c/2:(length(abs(s_mtd(:,1)))*ts*(tau/D/ts)-ts*(tau/D/ts))*c/2,abs(s_mtd)),xlabel('多普勒频率，单位：Hz'),ylabel('距离，单位：米'),zlabel('y(单位：伏)'),title('LFM MTD结果');
figure,mesh(1:fr/num_tongdao:fr,0:ts*c/2:(length(abs(s_mtd(:,1)))*ts-ts)*c/2,abs(s_mtd)),xlabel('多普勒频率，单位：Hz'),ylabel('距离，单位：米'),zlabel('y(单位：伏)'),title('LFM MTD结果');
