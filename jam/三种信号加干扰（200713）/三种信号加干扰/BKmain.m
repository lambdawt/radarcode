%主函数
function []=BKmain(fz,B,Gt,Gr,F,Pt,L,Pfa,R,v,rcsk,sigma0,Te,tau,tr,f0,f1,fs,frame,num_jilei,num_tongdao,num_cankao,num_baohu,model,number1,code) 
% clear all;
%  close all;
% clc;
global c;
...
figure,plot((0:fs/length(abs(fft(abs(fftshift(echo(1,:)))))):fs-fs/length(abs(fft(abs(fftshift(echo(1,:))))))),abs((fft(echo(1,:))))),xlabel('频率f(单位：Hz)'), ylabel('y(单位：伏/Hz)'),title('相位编码回波信号的频谱');
%生成噪声
...
figure,plot(0:ts:(N-1)*ts,real(s_echo_1(1:N))),xlabel('t(单位：s)'), ylabel('y(单位：伏)'),title('BK回波信号');
%检波及数字化
[s_echo_mf]=jianbo(s_echo_1,N,frame,f0,fs);
%脉冲压缩及降采样
[pc_result,pc_result1,M1]=maichongyasuo(s_echo_mf,M,frame,match_filter_fft,tau,D,ts);
figure,plot(0:ts:(M-1)*ts,(abs(pc_result1(1,:)))),xlabel('t(单位：s)'), ylabel('y(单位：dB)'),title('相位编码匹配滤波');
% s_pc_result=reshape(pc_result,1,M1*frame);
%动目标检测
[s_mtd]=mtd(pc_result1.',M1,num_jilei,num_tongdao);
figure,mesh(1:fr/num_tongdao:fr,0:ts*c/2:(length(abs(s_mtd(:,1)))*ts-ts)*c/2,abs(s_mtd)),xlabel('多普勒频率，单位：Hz'),ylabel('距离，单位：米'),zlabel('y(单位：伏)'),title('BK MTD结果');
